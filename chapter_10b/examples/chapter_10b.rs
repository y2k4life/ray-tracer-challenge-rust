use rustic_ray::{Camera, Color, Colors, Point, PointLight, patterns::Ring, Transformation, Vector, World, patterns::Checkers, patterns::Stripe, shapes::Plane, shapes::Shape, shapes::Sphere};
use std::{f64::consts::PI, fs::File, io::Write, path::Path};

fn main() {
    let mut world = World::new();

    let mut floor = Plane::new();
    floor.material.color = Color::new(1.0, 0.9, 0.9);
    floor.material.specular = 0.0;
    let mut pattern = Ring::new(Colors::WHITE, Colors::BLACK);
    pattern.transform = Transformation::new()
        .scale(0.5, 0.5, 0.5)
        .rotate_y(PI / 6.0)
        .build();
    floor.material.pattern = Some(Box::new(pattern));
    world.add_object(Box::new(floor));

    let mut middle = Sphere::new();
    middle.transform = Transformation::new().translate(-0.5, 1.0, 0.5).build();
    middle.material.diffuse = 0.7;
    let mut pattern = Stripe::new(Colors::RED, Colors::WHITE);
    pattern.transform = Transformation::new()
        .scale(0.1, 0.1, 0.1)
        .rotate_y(PI / 3.0)
        .rotate_y(PI / 4.0)
        .build();
    middle.material.pattern = Some(Box::new(pattern));
    world.add_object(Box::new(middle));

    let mut right = Sphere::new();
    right.set_transform(
        Transformation::new()
            .scale(0.5, 0.5, 0.5)
            .translate(1.5, 0.5, -0.5)
            .build(),
    );
    right.material.color = Colors::RED;
    right.material.diffuse = 0.7;
    right.material.specular = 0.3;
    world.add_object(Box::new(right));

    let mut left = Sphere::new();
    left.set_transform(
        Transformation::new()
            .scale(0.33, 0.33, 0.33)
            .translate(-1.5, 0.33, -0.75)
            .build(),
    );
    left.material.color = Color::new(1.0, 0.8, 0.1);
    left.material.diffuse = 0.7;
    left.material.specular = 0.3;
    let mut pattern = Checkers::new(Colors::RED, Colors::WHITE);
    pattern.transform = Transformation::new()
        .scale(0.1, 0.1, 0.1)
        .rotate_y(PI / 3.0)
        .rotate_y(PI / 4.0)
        .build();
    left.material.pattern = Some(Box::new(pattern));
    world.add_object(Box::new(left));

    world.light = Some(PointLight::new(
        Point::new(-10.0, 10.0, -10.0),
        Color::new(1.0, 1.0, 1.0),
    ));

    let mut camera = Camera::new(1280, 1080, PI / 3.0);

    camera.transform = Transformation::view_transform(
        Point::new(0.0, 1.5, -5.0),
        Point::new(0.0, 1.0, 0.0),
        Vector::new(0.0, 1.0, 0.0),
    );

    let canvas = camera.render(&world);

    let path = Path::new("chapter_10b.ppm");
    let display = path.display();

    let mut file = match File::create(&path) {
        Err(why) => panic!("couldn't create {}: {}", display, why),
        Ok(file) => file,
    };

    let ppm = canvas.canvas_to_ppm();
    match file.write_all(ppm.as_bytes()) {
        Err(why) => panic!("couldn't write to {}: {}", display, why),
        Ok(_) => println!("successfully wrote to {}", display),
    };
}
