...
impl World {
    ...
    /// let comps = i.prepare_computations(r, &vec![Intersection::new(2_f64.sqrt(), w.get_object(2).unwrap())]);
...
#[cfg(test)]
mod tests {
    use crate::{patterns::TestPattern, shapes::Plane, Material, Ray, Vector};
    ...
    pub fn shading_an_intersection_from_the_inside() {
        ...
        let comps = i.prepare_computations(r, &vec![Intersection::new(0.5, shape)]);
        let c = w.shade_hit(&comps, 1);
    ...
    fn shade_hit_is_given_an_intersection_in_shadow() {
        ...
        let comps =
            i.prepare_computations(r, &vec![Intersection::new(4.0, w.get_object(1).unwrap())]);
        let c = w.shade_hit(&comps, 1);
    ...
    fn the_reflected_color_for_a_nonreflective_material() {
        ...
        let comps =
            i.prepare_computations(r, &vec![Intersection::new(1.0, w.get_object(1).unwrap())]);
    ...
    fn reflected_color_reflective_material() {
        ...
        let comps = i.prepare_computations(
            r,
            &vec![Intersection::new(2_f64.sqrt(), w.get_object(2).unwrap())],
        );
        let color = w.reflected_color(&comps, 1);
    ...
    fn reflected_color_at_maximum_recursive_depth() {
        ...
        let comps = i.prepare_computations(
            r,
            &vec![Intersection::new(2_f64.sqrt(), w.get_object(1).unwrap())],
        );
        let color = w.reflected_color(&comps, 1);
