...
use rustic_ray::{shapes::Shape, shapes::Sphere, Canvas, Color, Point, Ray, Transformation};

fn main() {
    let mut shape = Sphere::new();
    draw_shape(&shape, "ch_05_circle.ppm");

    // shrink it along the y axis
    shape.transform = Transformation::new().scale(1.0, 0.5, 1.0).build();
    draw_shape(&shape, "ch_05_shrink_y.ppm");

    // shrink it along the x axis
    shape.transform = Transformation::new().scale(0.5, 1.0, 1.0).build();
    draw_shape(&shape, "ch_05_shrink_x.ppm");
    ...
    shape.transform = Transformation::new()
        ...
        .rotate_z(PI / 4.0)
        .build();
    draw_shape(&shape, "ch_05_shrink_rotate.ppm");
    ...
    shape.transform = Transformation::new()
        ...
        .build();
    draw_shape(&shape, "ch_05_shrink_skew.ppm");
}
